...
namespace ITI.Luxorna.Services
{
    public class TemplePackageService
    {
        ...
        public IEnumerable<TemplePackageViewModel> GetFilter(int NumberOfDayes)
        {
            var Departments = TemplePackageRepo.GetFilter(a => a.DayNumber == NumberOfDayes);
            return Departments.ToList().Select(i => i.toViewModel());
        }

        public void Remove(TemplePackageEditViewModel TemplePackage)
        {
            TemplePackageRepo.Remove(TemplePackage.toModel());
            unitOfWork.commit();
        }
    }
}
